...
export default class EventsClass {
	...
	public subscribeOnDelegateApprovalEvent(callback : (event : EventTypes.DelegateApproval) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			const _event: Record < string, any > = {};
			...
			callback(handleEventReturn(_event, getEventTypeDescription('DelegateApproval', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.DelegateApproval);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'DelegateApproval');
	}

	public subscribeOnReserveUsedAsCollateralEnabledEvent(callback : (event : EventTypes.ReserveUsedAsCollateralEnabled) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			const _event: Record < string, any > = {};
			...
			callback(handleEventReturn(_event, getEventTypeDescription('ReserveUsedAsCollateralEnabled', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.ReserveUsedAsCollateralEnabled);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'ReserveUsedAsCollateralEnabled');
	}

	public subscribeOnReserveUsedAsCollateralDisabledEvent(callback : (event : EventTypes.ReserveUsedAsCollateralDisabled) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			const _event: Record < string, any > = {};
			...
			callback(handleEventReturn(_event, getEventTypeDescription('ReserveUsedAsCollateralDisabled', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.ReserveUsedAsCollateralDisabled);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'ReserveUsedAsCollateralDisabled');
	}
